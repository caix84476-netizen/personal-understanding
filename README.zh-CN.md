...
现在很多 Agent 都自带"记忆"了——如果那个够你用，就用它。这个项目是给撞到它天花板的人准备的：

| | Agent 自带记忆 | Personal Understanding |
|---|---|---|
| 数据归属 | 锁在厂商账号里，基本导不出来，换工具就没了 | 你机器上的一个纯文本文件夹——能读、能 grep、能备份、能搬家 |
| 可迁移 | 记忆只在那个产品里生效 | 一份档案，任意 MCP 客户端通用——Claude、Codex、ZCode、VS Code，下一个工具照样用 |
| 可审计 | 黑盒——你不知道它存了什么，也不知道它为什么那么答 | 每条派生事实都能跳回原话；问"这个结论哪来的"，给你看来源 |
| 可控性 | 只有一个开关 | 政策归你——捕获门槛、权重、敏感度标签、回访规则都能改 |
| 检索 | 模糊摘要召回 | 三层检索，最终落到你自己的原话 |
| 隐私 | 你的个人历史躺在他们服务器上 | 纯本地——无遥测、无云调用 |

厂商记忆优化的是"对话更顺滑"。这个项目优化的是"**记忆归你、带得走、拿得出证据**"。这是两个不同的产品——厂商记忆做得再好，也不会让这个项目变多余，因为归属、迁移、审计这三件事从来不在厂商的优化目标里。
...
- **当前版本：v2.4.0**——两档调用（full/skip，活动足迹轮次并入完整档并受足迹纪律约束）；schema 稳定（`memory/v2/` v2.0.0），持续维护。[PyPI](https://pypi.org/project/personal-understanding/) 同步发布。
