...
- **Cloud (a WebDAV cloud drive, synced via rclone)**: every backup run pushes the whole local `backups/` directory to the cloud via `rclone copy` (stable + previous snapshots plus any pre-restore snapshot and log files that dir currently holds) — overwrite in place by name, no accumulating snapshot pairs, no resident background process. Two honest caveats (2.5.0 recon): (a) `rclone copy` only adds/overwrites, it never deletes remote files absent locally, so a snapshot name that disappears from `backups/` lingers on the drive until pruned manually; (b) the snapshot is a complete copy of the working archive, so recovery = download + extract, and the working archive directory itself is never pushed directly. The drive's free tier rate-limits WebDAV request frequency, and a few hundred small files can trigger 503 rate limiting. A 503 is temporary rate limiting; it clears itself in about half an hour and the next backup run resumes automatically;
...
- Recovery (revised 2026-09-02: scoped restore, never blind full overwrite): `python scripts/restore_stable.py --scope code|data|all`. Dry-run by default, sha-256 verified; `code` (default) restores code/docs only and never touches `memory/` or `sources/` — the common "the skill got broken" case; `data` restores the archive only (captures added after the snapshot date are lost — rescue them first); `all` is a full overwrite, only for whole-directory corruption. Before writing it snapshots every file it will overwrite into `backups/pre-restore-<timestamp>/`, so the restore itself is reversible; afterwards it auto-runs `rebuild_views.py` + `validate_memory.py`.
