...
- Recovery (revised 2026-09-02: scoped restore, never blind full overwrite): `python scripts/restore_stable.py --scope code|data|all`. Dry-run by default, sha-256 verified; `code` (default) restores code/docs only and never touches `memory/`/`sources/`; `data` restores the archive only; `all` is a full overwrite for whole-directory corruption. It snapshots overwritten files into `backups/pre-restore-<timestamp>/` before writing and auto-runs `rebuild_views.py` + `validate_memory.py` afterwards.
