...
SALIENCE_LABELS = {3: "pivotal", 2: "key", 1: "supporting", 0: "passing"}
FOLLOWUP_OPEN_STATUSES = {"pending", "due", "overdue"}
# Example redirects (old id -> canonical id) left by an entity merge.
# Customize for your own archive after merging duplicate entities.
ENTITY_REDIRECTS = {
    "entity.friend.best-friend": "entity.friend.close-friend-a",
    "entity.friend.old-classmate": "entity.friend.close-friend-b",
    "entity.relationship.ambiguous-person": "entity.relationship.long-acquaintance",
}
...
def write_text_atomic(path: Path, content: str) -> None:
    """ tmp+rename write; a mid-process crash never leaves a half-written file."""
...
def followup_is_due(row: dict[str, Any], today: str | None = None) -> bool:
    """The archive-wide single "is due" test: dates are truncated to the day so a due_at carrying a time component cannot fail to match forever."""
...
def date_info(meta: dict[str, str]) -> dict[str, Any]:
    ...
    return {"date_start": None, "date_end": None, "date_precision": "unknown", "date_basis": "undated", "date_text": "undated"}
...
def record_title(meta: dict[str, str], body: str) -> str:
    ...
    return (sentence[:40] + "…") if len(sentence) > 40 else (sentence or meta.get("id", "unnamed"))
...
def capture_records() -> list[dict[str, Any]]:
    ...
    for meta_file in sorted(attachment_meta_files):
        ...
        descriptor = f"[original attachment: {meta.get('original_filename') or Path(source_path).name}; type: {content_type}; SHA256: {meta.get('sha256') or ''}]"
...
def build_fragments(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, list[str]]]:
    ...
    for row in rows:
        ...
        fragments.append(make_fragment(fid, body, split_ids(meta.get("source_refs")), "legacy-derived-record", "summary_only", record_refs=[record_id], fragment_type="legacy-summary", note="Legacy derived record; must not be treated as user verbatim."))
    ...
    for capture in capture_records():
        ...
        fragment = make_fragment(fid, capture["text"], [capture["path"]], "conversation-capture", "exact_attachment" if is_attachment else "verbatim", speaker=capture["speaker"], fragment_type="raw-attachment" if is_attachment else "raw-message", locator={"capture_id": capture["capture_id"], "path": capture["path"], "metadata_path": capture.get("metadata_path")}, authoritative=capture["speaker"] == "user", note="Immutable original attachment; readable text requires an explicitly linked OCR/transcription derivation." if is_attachment else "Immutable conversation verbatim.", attachment_sha256=capture.get("sha256") if is_attachment else None, content_type=capture.get("content_type") if is_attachment else None, byte_length=capture.get("byte_length") if is_attachment else None)
...
# Example seed entities used for mention detection while deriving v2 entities
# from legacy text. Replace them with the people, places, works, and concepts
# that appear often in YOUR archive so older summaries gain entity links.
SEED_ENTITIES = [
    ("entity.school.example-university", "Example University", "school_or_organization", ["Example University"]),
    ("entity.program.example-field", "example field of study", "concept", ["example field"]),
    ("entity.interest.running", "running", "game_or_media", ["running", "jogging"]),
    ("entity.school.high-school", "high school", "school_or_organization", ["high school", "senior year"]),
    ("entity.school.middle-school", "middle school", "school_or_organization", ["middle school"]),
    ("entity.object.laptop", "laptop", "object", ["laptop", "computer"]),
    ("entity.object.headphones", "headphones", "object", ["headphones", "earbuds"]),
    ("entity.book.example-novel", "an example novel", "book_or_work", ["example novel"]),
    ("entity.game.game-library", "game library", "game_or_media", ["Steam", "game library", "games"]),
    ("entity.environment.home", "family and home environment", "environment", ["family", "home", "living environment"]),
    ("entity.concept.autonomy", "autonomy", "concept", ["autonomy"]),
    ("entity.concept.dignity", "dignity", "concept", ["dignity"]),
    ("entity.concept.ai-agi", "AI and AGI", "concept", ["AI", "AGI"]),
    ("entity.place.hometown", "hometown", "place", ["hometown"]),
]
...
def alias_matches(text: str, alias: str) -> bool:
    """Pure Latin/digit aliases match on word boundaries to avoid substring false positives such as "AI" hitting "haircut"; aliases containing Chinese or other non-ASCII characters still match by substring."""
    alias = alias.strip()
    if len(alias) < 2:
        return False
...
def build_entities(rows: list[dict[str, Any]], fragments: list[dict[str, Any]], record_fragment: dict[str, list[str]]) -> list[dict[str, Any]]:
    ...
    for entity_id, label, entity_type, aliases in SEED_ENTITIES:
        ...
        entities[entity_id]["notes"].append("v2 migration seed: a lightweight entity card created from an explicit name in an old summary; details still need source verification.")
    for old_id, row in redirected:
        ...
        entities[canonical]["notes"].append(f"merged aliases and material from {old_id}; see relations for the redirect.")
    for row in rows:
...
def build_entries(rows: list[dict[str, Any]], record_fragment: dict[str, str], entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        entries.append({"id": f"entry.{slug(record_id)}", "record_id": record_id, "title": record_title(meta, body), "entry_kind": meta.get("kind", "unknown"), "status": meta.get("status", "current"), "confidence": meta.get("confidence", ""), "sensitivity": meta.get("sensitivity", ""), "summary": clean_text(body), "salience": salience, "salience_label": SALIENCE_LABELS[salience], "salience_basis": basis, **d, "phase": meta.get("phase") or "unphased", "domain": meta.get("domain") or "domain.unclassified", "entity_refs": sorted(set(entity_refs)), "unresolved_referents": [], "fragment_refs": record_fragment.get(record_id, []), "record_refs": [record_id], "before_ids": [], "after_ids": [], "relation_refs": {field: split_ids(meta.get(field)) for field in ("related_ids", "supports", "contradicts", "supersedes")}, "legacy_import": True, "note": "From a v0.6 derived record; salience is an import heuristic and can be re-verified against verbatim captures later."})
...
def build_contexts(entities: list[dict[str, Any]], entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (entity_id, domain), group in sorted(grouped.items()):
        ...
        contexts.append({"id": context_id, "kind": "entity-context", "entity_id": entity_id, "entity_ids": [entity_id], "context_key": domain, "label": domain.replace("domain.", ""), "entry_refs": sorted({entry["id"] for entry in group}), "related_entity_ids": related, "facet_policy": "canonical-fragments-with-linked-projections", "note": "Context slices never duplicate facts; other records navigate through this card."})
    ...
    for (left, right), group in sorted(pair_grouped.items()):
        ...
        contexts.append({"id": context_id, "kind": "facet", "entity_id": None, "entity_ids": [left, right], "context_key": f"{left} x {right}", "label": f"{left_label} × {right_label}", "entry_refs": sorted({entry["id"] for entry in group}), "related_entity_ids": [left, right], "facet_policy": "canonical-fragments-with-linked-projections", "note": "Cross-entity context card; each fact is stored once, and entity profiles use this card to return to the shared story."})
...
def build_followups() -> list[dict[str, Any]]:
    """followups.jsonl is the authoritative state; open-loops.md is only a seed source waiting to be imported.

    Every build appends open loops from open-loops.md that have not been imported yet (idempotent);
    closed (answered/declined) loops are never imported; manual edits in the jsonl are never overwritten.
    """
...
def write_pages(entities: list[dict[str, Any]], entries: list[dict[str, Any]], fragments: dict[str, dict[str, Any]], contexts: list[dict[str, Any]]) -> None:
    ...
    for entity in entities:
        lines = [f"# {entity['label']}", "", f"- Type: {entity['entity_type']}", f"- Identity status: {entity['identity_status']}", f"- Mention count: {entity['mention_count']}", "", "## Direct material", ""]
        for fragment_id in entity.get("fragment_refs", []):
            fragment = fragments.get(fragment_id)
            if fragment: lines.append(f"- [{fragment['fidelity']}] {fragment['verbatim']}")
        lines.extend(["", "## Related stories (connections are never deleted)", ""]); seen: set[str] = set()
        ...
        lines.extend(["", "## Notes", "", "Each fact is stored once; other people, places, schools, books, games, and concepts navigate through context cards. The archive never deletes social connections for the sake of 'purity'.", ""])
    ...
    for entry in entries:
        lines = [f"# {entry['title']}", "", f"- Date: {entry['date_text']}", f"- Salience: {entry['salience_label']} ({entry['salience']})", f"- Type: {entry['entry_kind']}", "", "## Evidence", ""]
        for fragment_id in entry.get("fragment_refs", []):
            fragment = fragments.get(fragment_id)
            if fragment: lines.extend([f"> {fragment['verbatim']}", f"> Source: {', '.join(fragment.get('source_refs', []))}; fidelity: {fragment.get('fidelity')}", ""])
        lines.extend(["## Related entities", "", *[f"- `{ref}`" for ref in entry.get("entity_refs", [])], ""])
...
def _read_json_object(path: Path) -> tuple[dict[str, Any], str | None]:
    """Read a JSON object; on corruption return an empty object plus an error description instead of raising."""
    if not path.exists():
        return {}, None
    try:
    ...
    return (value if isinstance(value, dict) else {}), None if isinstance(value, dict) else f"{path.name}: top level is not a JSON object"
...
def v2_audit(strict: bool = False) -> dict[str, Any]:
    ...
    if legacy_capture_debt:
        warnings.append({"code": "legacy-missing-verbatim-capture", "count": len(legacy_capture_debt), "sample": legacy_capture_debt[:12], "action": "Old summaries cannot reliably recover the verbatim text; future updates must capture first"})
