...
def tool_definitions() -> list[dict[str, Any]]:
    return [
        {"name": "personal_preflight_turn", "description": "当前用户消息的强制内容预检。它持久化 turn receipt；个人经历、感受、关系、偏好、决定即使请求形式是润色/总结/看图，也会要求 capture。三档调用：内容含个人材料走完整档（tier=full/auto）；模型判定回答可沉淀活动足迹（如'正在玩某游戏'）时声明 tier=light 走轻量补记；纯技术/吃喝/购物等明显无关或零增值轮次不调用本工具（跳过档）。", "inputSchema": {"type": "object", "properties": {"text": {"type": "string"}, "turn_id": {"type": "string"}, "conversation_id": {"type": "string"}, "tier": {"type": "string", "enum": ["auto", "full", "light", "skip"], "description": "模型显式声明档位；auto=纯内容分类，light=轻量补记档，skip=强制跳过，full=完整档兜底"}}, "required": ["text"], "additionalProperties": False}},
        ...
        {"name": "personal_add_record", "description": "创建派生记录。若来源是当前用户补充，必须先有 verbatim capture，并把 verbatim_refs 写入记录。轻量档补记传 tier=light（salience 0-1 的微型记录），完整档记录不传或传 full。", "inputSchema": {"type": "object", "properties": {"id": {"type": "string"}, "kind": {"type": "string", "enum": sorted(VALID_KINDS)}, "domain": {"type": "string"}, "summary": {"type": "string"}, "tier": {"type": "string", "enum": ["full", "light"], "description": "记录档位：light=轻量补记（活动足迹类微型记录，salience 0-1）；缺省/full=完整档"}, "source_refs": {"type": "string"}, "verbatim_refs": {"type": "string"}, "capture_id": {"type": "string"}, "confidence": {"type": "string", "enum": sorted(VALID_CONFIDENCE), "default": "high"}, "sensitivity": {"type": "string", "enum": sorted(VALID_SENSITIVITY), "default": "ordinary"}, "related_ids": {"type": "string"}, "aliases": {"type": "string"}, "salience": {"type": "integer", "minimum": 0, "maximum": 3}, "phase": {"type": "string"}, "valid_from": {"type": ["string", "null"]}, "last_confirmed": {"type": ["string", "null"]}, "date_end": {"type": ["string", "null"]}, "date_precision": {"type": "string"}, "date_basis": {"type": "string"}, "entity_refs": {"type": "string"}, "record_role": {"type": "string"}}, "required": ["id", "kind", "summary"], "additionalProperties": False}},
        {"name": "personal_finalize_capture", "description": "关闭当前原话捕获的派生闭环。derived 必须已有至少一条双向链接记录；无需派生时必须写具体原因。轻量档（tier=light）的轮次不能以 no-derivation-needed 收尾，session_check 会 fail closed。回答前必须调用。", "inputSchema": {"type": "object", "properties": {"capture_id": {"type": "string"}, "disposition": {"type": "string", "enum": ["derived", "no-derivation-needed"]}, "reason": {"type": "string"}}, "required": ["capture_id", "disposition"], "additionalProperties": False}},
...
def add_record(data: dict[str, Any]) -> dict[str, Any]:
    record_id = str(data.get("id", "")).strip(); kind = str(data.get("kind", "")).strip(); summary = str(data.get("summary", "")).strip(); confidence = str(data.get("confidence", "high")).strip(); sensitivity = str(data.get("sensitivity", "ordinary")).strip(); domain = str(data.get("domain", "")).strip(); tier = str(data.get("tier", "")).strip() or "full"; source_refs = [ref.strip() for ref in re.split(r"[;,]", str(data.get("source_refs", ""))) if ref.strip()]; verbatim_refs = [ref.strip() for ref in re.split(r"[;,]", str(data.get("verbatim_refs", ""))) if ref.strip()]; capture_id = str(data.get("capture_id", "")).strip()
    ...
    if tier not in {"full", "light"}: return text_result("拒绝写入：tier 只能是 full 或 light。", error=True)
    if not summary: return text_result("拒绝写入：summary 不能为空。", error=True)
    if tier == "light" and int(data.get("salience", 1) or 1) > 1: return text_result("拒绝写入：轻量补记（tier=light）的 salience 不得超过 1。", error=True)
    ...
    lines = ["---", f"id: {record_id}", f"kind: {kind}", "status: current", f"confidence: {confidence}", f"sensitivity: {sensitivity}", f"tier: {tier}"]
