...
probe 与 routing 共用同一套加权排序（2.4.1 起，2.5.0 起为 weighted-idf-2-aliasfloor）：IDF 稀有词加权（"只狼""猫学派"这类短专有名词权重高，烂大街单字权重接近零）+ 长度归一（超长记录不再靠字数霸榜）+ 实体别名路由（命中实体的时间条目获得反哺加权，如"哥哥 家里"→哥哥冲突）+ 单字实体别名豁免（"妈""她"这类档案别名不按噪音降权，口语查询可直接触达家族簇）+ 内容词入选门槛（只被非别名单字偶然命中的记录不占检索名额，杜绝"防/钱/状态"式捞偏）。因此短词游戏名等查询的漏召回已修复，定向查重直接按足迹关键词 probe 即可；routing 仍是按领域浏览的全量地图，供查不到关键词时兜底。
