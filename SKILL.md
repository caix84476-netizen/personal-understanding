...
内容分类器是关键词启发式，两层触发（2.5.0 §8）：第一人称+标记词（"我+任何经历/状态词"），以及**强情感词独立触发**——中文常态省略主语，"有点烦""挺焦虑的""心情很差"这类抒发/状态轮次没有"我"字也必须召回（带技术语境否决：句中含 python/代码/配置/报错等词时强情感词不触发，"这个 bug 烦死了"仍是技术轮）。诚实边界：纯枚举不可持续——"兵役登记搞完了"这种省略主语的生活事件陈述、"最近状态不太好"这种"状态"撞技术词的弱表达，关键词表按定义抓不全，**兜底责任在模型**：判断为个人材料就在同一 turn-id 上重声明 `--tier full`（2.5.0 §8.1 后同 id 升级即时生效，不再返回假 receipt）。

**实测已知的漏型清单（2.6.1，三轮泛化测出；词表法到头，模型兜底时优先怀疑这些形态）**：省略主语的情感弱表达（"心里堵得慌""说不上来为啥""越想越尴尬""被拒了心里不是滋味""诸事不顺"——已补词但形态无穷）、以事件陈述包裹的情绪（"兵役登记搞完了"）、"状态/感觉"类弱词组合。规则不变：**凡是"这句话里有人的温度"就按完整档走**，判错方向宁可多 capture（no-derivation 收场无成本）也不要漏 capture（用户原话永久丢失）。
...
2. **跨域收集**：允许每条线各取 timeline/knowledge 前排 + associations 的强候选（via 清晰者），禁止单通道凑数；
